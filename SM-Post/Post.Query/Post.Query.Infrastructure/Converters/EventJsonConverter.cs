    ...
    public class EventJsonConverter : JsonConverter<BaseEvent>
    {
        ...
        public override BaseEvent? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            if(!JsonDocument.TryParseValue(ref reader, out var doc)) {
                throw new JsonException("Failed to parse event");
            }
            if(!doc.RootElement.TryGetProperty("Type", out var type))
            {
                throw new JsonException("Couldnt not detect the type discriminator  property");
            }
            var typeDsicriminator = type.GetString();
            var json = doc.RootElement.GetRawText();

            return typeDsicriminator switch
            {
                ...
                _ => throw new JsonException("notsuported event")
            };
        }
